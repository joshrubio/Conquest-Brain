...
- **1. Candidate pull (`tools/pull_assets.py`):** write `07-pull.tsv` (one row per beat that needs an image/clip: beat, kind `stock|archive|video`, source, query, opts; `n=3` total per beat by default). `python tools/pull_assets.py E0XX-slug` hits the free APIs (Met, AIC, Pexels, Pixabay, Unsplash, Openverse) and writes **`07-candidates.md`** (git record) + **`07-candidates.html`** (thumbnail picker). Keys in `tools/.env` (gitignored); keyless sources still run without it.
- **2. Photography pass (Josh, manual):** open `07-candidates.html` in a browser, judge each thumbnail on resolution (vs. the template standard) / condition / colour / crop / sequence coherence against the series look ([docs/03](03-brand-identity.md) §Visual direction), tick the keepers, hit **Exportar 07-picks.txt** (saves into the episode folder). Then `python tools/pull_assets.py E0XX-slug --download` pulls the picks into `assets/`, verifies real resolution, appends `assets/CREDITS.md`, prints manifest rows. (No browser? tick `- [x]` in `07-candidates.md` instead.)
