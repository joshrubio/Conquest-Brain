...
Working analysis that feeds the `brain/` specs. Not published, not part of any episode.

## dieck-docs/

Reverse-engineering the reference format (Farid Dieck — *Dieck Docs*) so that `brain/02-content-format.md`, `brain/08-tone-of-voice.md` and `brain/09-reflection-rules.md` are grounded in real structure, not assumption.
...
- `dieck-docs/structure-analysis.md` — the analysis worksheet. One block per video + synthesis. Feeds `brain/02`, `08`, `09`.
...
4. Update the three `brain/` files; bump their `Status: v0` line to `v1 — validated against N transcripts (date)`.
