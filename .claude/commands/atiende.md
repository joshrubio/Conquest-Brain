...
4. Por cada entrada `{ep, stage, action, reads, rules, produces, note}`:
   ...
   - **Al entrar en Stage 7** (`advance.py next` devuelve «abre la revisión de Stage 7», sin entrada de cola propia): el *pull* es mecánico y va antes que la revisión humana, no en vez de ella. Escribe `07-pull.tsv` (una fila por beat `archivo`/`stock` de la espina de `06-shotlist.md` — nunca los `ia`/`gráfico`/`acamara`/`negro`, `brain/12`) y corre `python tools/pull_assets.py <slug>` para generar `07-style-pass.html`. Sin `07-pull.tsv` ese comando falla con `sys.exit` — no es un bug, es el paso que falta. Si el aviso final lista beats «sin candidatos», afina la query y re-corre solo esos con `--beats N,N` (máx. 1 ronda de más); si siguen vacíos, conviértelos a `tipo: ia` en `06-shotlist.md` en vez de dejarlos sin fila (`brain/12`). A partir de ahí para — elegir candidatos y «Finalizar Stage 7» es de Usuario 001.
