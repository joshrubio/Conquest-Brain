...
- **Follows `brain/15`'s label rule, not a separate ban:** a photoreal AI recreation of a document/record or of a real person's likeness is allowed here too, on the same terms — always labelled `Representación pictórica`, never presented as the genuine rights-managed item itself (`brain/15` governs AI illustration regardless of this tier).
...
- **Brief means brief:** a few seconds per clip; as a rule no single fragment over ~5 s, never two fragments of the same scene back-to-back, and no more than ~15 % of the runtime in copyrighted film footage in total ([brain/20](20-experimental-clip-protocol.md) §4.4 has the full rule). If the edit needs more than that, the beat is leaning on someone else's work — rebuild it.
- **Never load-bearing:** a copyrighted clip never carries a beat on its own. Narration runs over it, original audio always stripped ([brain/16](16-edit-and-delivery.md) §The soundtrack), cut into the case-file device, graded into the house look. A raw, unedited excerpt playing clean is out — same transformation bar as 1b.
...
- **Follows `brain/15`'s label rule, not a separate ban:** a photoreal AI recreation of a document/record or of a real person's likeness is allowed here too — always labelled `Representación pictórica` (`brain/15` governs regardless).
