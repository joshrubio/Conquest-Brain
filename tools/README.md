...
| `pickup_room.py` | 9 | Pickup UI for script lines the take doesn't cover: `<take>.pickups.json` → record the line, match its loudness to the original (`match_audio.py`), fold it in on the next apply. Audio only. |
...
| `match_audio.py` | 9 | Conditions a pickup recording to sit next to the original take: two-pass loudness match to a reference window + gentle highpass + edge fades. Called by `pickup_room`/`serve.py`. ffmpeg. |
...
| `beat_ops.py` | 9 | Structural edits on the schema-2 timeline, by stable beat `id`: add / split / merge / duplicate / delete / reorder / audio cut, plus **cut undo/redo** (a timestamped journal, `09-cut-journal.json`) and **resync**. Driven by the edit room; also a CLI. |
| `beat_asset.py` | 9 | Asset provenance for the edit room: list the library, point a beat at an existing asset or fetch a new one (URL/path), log the source-log stub. |
| `migrate_timeline.py` | 9 | One-off: migrate a legacy schema-1 timeline to the authored schema 2. |
| `undo_take.py` | 8 | Undo a Stage 8 take upload: delete the take and what Stage 9 generated from it, put the episode back at Stage 8. |
| `delete_episode.py` | — | Discard an episode entirely; moves it to `episodes/_trash/` instead of deleting. |
| `awake_until.py` | — | Keep Windows from idle-sleeping until the given PIDs exit (for a job started without `keep_awake`). |
...
| `mediabin.py` | — | locates `ffmpeg` / `ffprobe` (PATH → `static-ffmpeg` → `imageio-ffmpeg`), plus the load helpers every heavy job shares: `keep_awake()`, `lower_priority()` (below-normal, inherited by ffmpeg) and `cpu_threads()` (60 % of cores) — imported by the Stage-9 tools, not a CLI. |
