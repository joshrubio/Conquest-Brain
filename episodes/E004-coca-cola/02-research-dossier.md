# Dossier de investigación — E004 «Coca-Cola»

> Stage 2. Acompaña a `03-source-log.csv`. No se pasa a guion hasta que cada afirmación de carga tenga ≥1 fuente Tier A/B.
> **Nota de esta pasada:** dossier construido en una sesión de investigación web (WebSearch/WebFetch), como marca `brain/06` para Stage 2. Varias afirmaciones descansaban en Tier C (Wikipedia, usada como pista) — se listan explícitamente en la sección 6. **Revisado y aprobado por Usuario 001 en `02-research.html` (2026-09-12)** — el gate se firma tal cual; la sección 6 se deja como registro de lo que quedaba en Tier C en el momento del visto bueno, útil para el guionista y para el fact-check de Stage 5, no como bloqueo.

| Campo | Valor |
|-------|-------|
| ID episodio | E004 |
| Investigador principal | Claude (borrador) — revisado y aprobado por Usuario 001 |
| Última actualización | 2026-09-12 |
| Nº de fuentes en source-log | 6 (A: 1 / B: 2 / C: 2 / D: 1 — la D es la fuente corporativa, citada como objeto, nunca como respaldo) |

## 1. Reinicio de fuentes (si aplica)

No aplica — la idea viene del pool público (`ideas/idea-pool.md`, DOC-01), generada de material histórico documentado. Ninguna conexión privada (`brain/05`).

## 2. Cronología

| Fecha | Hecho | Fuente(s) [ID source-log] | Tier | Nota / certeza |
|-------|-------|----------------------------|------|-----------------|
| 1831-01-08 (o 07-08, las fuentes varían) | Nace John Stith Pemberton, Knoxville, Georgia | S01, S03 | B/C | **Discrepancia menor**: New Georgia Encyclopedia da enero, Wikipedia da julio — verificar registro de nacimiento antes de guion |
| abril de 1865 | Herido (sable, según S02/S03; New Georgia Encyclopedia solo dice "casi muere en combate" sin especificar arma) en Columbus, Georgia, Guerra Civil | S01, S02, S03 | B/C | Arma exacta (sable vs. otra) — confirmar contra S02 con página |
| 1850s-1880s | Carrera como farmacéutico/boticario en Columbus y luego Atlanta; funda varios negocios y laboratorios; forma parte de la primera junta de licencias farmacéuticas de Georgia (1881-1887) | S01 | B | Sólido — enciclopedia estatal |
| — (post-1865, sin fecha exacta) | Adicción a la morfina, usada para el dolor de la herida; empieza a experimentar con tónicos propios buscando una alternativa no adictiva | S02, S03 | B/C | Núcleo del ángulo del episodio — subir a Tier A/B firme con Pendergrast (página) antes de Stage 4 |
| 1885 | Lanza "Pemberton's French Wine Coca" — vino con extracto de coca y nuez de cola, inspirado en el Vin Mariani francés; vendido como tónico nervioso/remedio para dolores de cabeza | S02, S03 | B/C | — |
| 1886 (Atlanta declara prohibición local del alcohol) | Pemberton retira el vino de la fórmula, sustituye por sirope de azúcar, y la rebautiza «Coca-Cola» — la vende como bebida de temperancia | S02, S03, S06 | B/C/A | S06 (LOC) confirma mayo de 1886 como fecha del primer Coca-Cola servido (farmacia Jacobs') |
| 1886 | Frank Mason Robinson (socio/contable de Pemberton) sugiere el nombre «Coca-Cola» y diseña el logo en script Spencerian | S05 | C | Subir de tier — es un dato muy citado, casi seguro verificable en prensa/registro de marca |
| 1887 (termina la prohibición de Atlanta) | Pemberton retoma la producción del French Wine Coca (con alcohol), que sigue vendiéndose e incluso supera en ventas a la nueva Coca-Cola hasta su muerte | S02 | B | **Matiz importante para el guion**: la "conversión" a la fórmula sin alcohol no fue limpia ni definitiva en vida de Pemberton — complica la narrativa simple de "prohibición → Coca-Cola para siempre" |
| 1887-1888 | Enfermo y con problemas de dinero, Pemberton vende participaciones de los derechos de fabricación a **varias partes distintas y superpuestas**: Margaret Dozier y Woolfolk Walker, luego J. C. Mayfield (un tercio de "Pemberton Medicine Company"), y también a Asa Candler — generando un "chain of title" disputado | S05 | C | **Pista de alto valor, sin confirmar en Tier A** — si se sostiene, es un beat mejor que "vendió todo a Candler y punto"; localizar el pleito judicial (Mayfield v. Candler o similar) antes de escribirlo en el guion |
| 1888-08-16 | Muere John Pemberton en Atlanta, pobre y todavía adicto a la morfina | S01, S02, S03 | B/C | Fecha de muerte sólida (S01 la confirma independientemente) |
| 1888 (marzo) | Se presenta la petición de incorporación de "The Coca-Cola Company" — antes de la muerte de Pemberton | S01 | B | — |
| 1888-1891 | Asa Candler consolida progresivamente la propiedad total de la fórmula y la marca (comprando o resolviendo las participaciones superpuestas), por un total citado de $2.300 | S02, S04 (como fecha, no como respaldo del relato completo) | B | La cifra de $2.300 aparece también en la propia web corporativa (S04) — coincide con la literatura secundaria |
| 1892 | Se incorpora formalmente "The Coca-Cola Company" (Candler + hermano John S. Candler + Frank Robinson + 2 socios más); ventas del sirope crecen ~10× bajo el marketing de Candler | S02, S04 | B/D | S04 es la fuente corporativa — usar solo la fecha/estructura, no su marco narrativo |
| ~1901-1903 | Se retira progresivamente la cocaína de la fórmula (presión de la prensa/opinión anticocaína de la era progresista); se sustituye por hoja de coca "decocainizada" (el extracto sin el alcaloide, por Stepan Company desde principios del s. XX) para conservar el sabor | S02 | B | Fecha con cierto rango en las fuentes — fijar el año exacto contra Pendergrast antes de afirmarlo con precisión en pantalla |

## 3. Personas / entidades clave

| Nombre | Rol | Vivo/fallecido | Notas de sensibilidad legal | Fuentes |
|--------|-----|-----------------|-------------------------------|---------|
| John Stith Pemberton | Farmacéutico, inventor de la fórmula (1886) | Fallecido (1888) | Ninguna — figura histórica, sin herederos directos conocidos por el equipo | S01, S02, S03 |
| Asa Griggs Candler | Comprador de los derechos, fundador de The Coca-Cola Company (1892) | Fallecido (1929) | Ninguna — tratar sin caricaturizar (`brain/09` A7): documentar lo que hizo, no inventarle motivos internos | S01, S02, S04 |
| Frank Mason Robinson | Socio/contable de Pemberton; acuñó el nombre y el logo | Fallecido | Ninguna | S05 |
| J. C. Mayfield / Margaret Dozier / Woolfolk Walker | Compradores de participaciones superpuestas en los derechos, 1887-88 | Fallecidos | Ninguna — verificar bien antes de nombrarlos, es la parte menos confirmada del dossier | S05 |
| The Coca-Cola Company (moderna) | Corporación pública actual | — (entidad, no persona) | No es el sujeto del episodio — cero branding moderno en pantalla (`brain/04` §4, ya en el brief) | S04 |

## 4. Afirmaciones de carga (load-bearing) y su respaldo

| # | Afirmación (como irá, en esencia) | Fuente(s) [ID] | Tier | Estado |
|---|-----------------------------------|----------------|------|--------|
| C1 | Pemberton fue herido en la Guerra Civil (Columbus, abril 1865) y quedó adicto a la morfina | S01, S02, S03 | B/C | pendiente — confirmar arma exacta y página de Pendergrast |
| C2 | Antes de Coca-Cola vendía French Wine Coca (vino + coca + cola), inspirado en el Vin Mariani | S02, S03 | B/C | pendiente — subir de tier |
| C3 | La prohibición de Atlanta de 1886 empuja la versión sin alcohol, rebautizada Coca-Cola | S02, S03, S06 | A/B/C | verificada (S06 es Tier A institucional) |
| C4 | Frank Robinson acuñó el nombre y el logo | S05 | C | pendiente — subir de tier |
| C5 | Pemberton siguió vendiendo la versión alcohólica (más vendida que Coca-Cola) hasta morir | S02 | B | pendiente — confirmar página |
| C6 | Pemberton vendió participaciones superpuestas/conflictivas a varias partes antes de morir, no una venta limpia a Candler | S05 | C | **pendiente fuerte** — necesita Tier A (registro judicial) antes de entrar al guion con esa forma; si no se confirma, usar la versión más simple y menos específica |
| C7 | Candler consolidó la propiedad total hacia 1891 por ~$2.300 | S02, S04 | B/D | verificada en dos fuentes independientes (secundaria + la propia empresa) |
| C8 | Pemberton murió pobre y adicto en 1888, antes de que la marca despegara | S01, S02, S03 | B/C | verificada — S01 confirma fecha independientemente |
| C9 | La cocaína se retiró de la fórmula hacia 1901-1903 | S02 | B | pendiente — fijar año exacto |
| C10 | La página oficial de la empresa no menciona la adicción, la cocaína, ni la disputa de derechos | S04 | D (autoverificado por lectura directa de la página) | verificada — es una observación directa de lo que la página dice/omite, no una cifra a sostener con tier |

**Ninguna C está aún en el estándar final del gate** (≥1 Tier A/B firme, sin depender de Tier C) — C1, C2, C4, C5, C6, C9 necesitan una segunda ronda contra prensa primaria (Chronicling America / LOC), Pendergrast con página, o el registro judicial antes de Stage 4.

## 5. Puntos disputados / versiones en conflicto

| Tema | Versión A (fuente) | Versión B (fuente) | Cómo lo trataremos en pantalla |
|------|---------------------|----------------------|-----------------------------------|
| Fecha de nacimiento de Pemberton | 8 de enero de 1831 (New Georgia Encyclopedia, S01) | 8 de julio de 1831 (Wikipedia, S03) | Verificar contra un registro civil/genealógico antes de citar un día exacto; si no se resuelve, dar solo el año en pantalla |
| Cómo se vendieron los derechos a Candler | Una venta relativamente directa, consolidada por etapas hasta 1891 (S02, S04) | Una venta caótica con participaciones superpuestas vendidas a varias partes a la vez (S05) | Si se confirma la versión B en fuente Tier A, es el ángulo más fiel e interesante — «incluso la venta de los derechos fue un caos, no la transacción limpia que se cuenta» |
| Retirada exacta de la cocaína | ~1901 en algunas fuentes | ~1903 en otras (por presión de prensa) | Enmarcar con salvedad explícita en pantalla ("hacia 1901-1903, las fuentes varían en el año exacto") en vez de fijar un año falso con precisión |

## 6. Huecos y preguntas abiertas

- **Fuente Tier A para la adicción a la morfina y el detalle de la herida** — hoy descansa en Pendergrast (sin página verificada) + Wikipedia (Tier C). Buscar: registros médicos/pensión de veterano, o el propio Pendergrast con cita de página exacta.
- **El pleito Mayfield/Candler (o el reparto de derechos en general)** — es la pista narrativamente más valiosa del dossier (C6) y la menos confirmada. Localizar el registro judicial real (sería Tier A) antes de escribir esa escena.
- **Prensa de Atlanta 1885-1888** — no se ha buscado aún directamente en Chronicling America (LOC) ni en archivos de Georgia; sería la fuente Tier A más directa para los anuncios de "French Wine Coca" y la ordenanza de prohibición de 1886 citada en el brief.
- **Fecha exacta de retirada de la cocaína** — fijar el año antes de citarlo con precisión en pantalla.
- **Frank Robinson** — confirmar la anécdota del nombre/logo contra una fuente mejor que Wikipedia (hay varios sitios de historia de marcas que la repiten, pero conviene una fuente con nota al pie).

## 7. Citas textuales candidatas

| Cita (verbatim) | Autor | Fecha | Fuente [ID] + localización exacta | ¿Traducción propia? |
|-------------------|-------|-------|--------------------------------------|------------------------|
| «Sole ownership, which Mr. Candler did not actually achieve until 1891, cost a total of $2,300.» | The Coca-Cola Company (página institucional) | consultado 2026-09-12 | S04, página "The Asa Candler Era" | Sí — traducción propia al citarla en pantalla, marcada como cita de la propia empresa |

*(Pendiente localizar una cita textual verificada y verbatim de Pemberton o de un periódico de época — ninguna de las fuentes de esta ronda trae una cita primaria citable con localización exacta.)*

## 8. Ángulo psicológico / humano

- **Mecanismo (registro práctico, per `brief`):** el relato oficial de una marca/institución es curaduría, no historia completa — lo incómodo se edita de la versión que la propia empresa cuenta de su origen.
- **Anclaje concreto:** S04 (la página oficial de Candler) frente a S02/S05 (la adicción, la cocaína, la disputa de derechos) — el contraste directo entre lo que la empresa cuenta y lo que el registro histórico documenta es el anclaje, no una teoría psicológica abstracta que necesite un teórico citado aparte. Esto significa que el cierre probablemente **no necesita** un anclaje `brain/09 §A6` de un autor externo (a diferencia de E003) — es una observación editorial/práctica directamente demostrada por el contraste de fuentes, no una afirmación de mecanismo psicológico que requiera respaldo teórico. Revisar en Stage 3/4 si el registro pide reforzarlo con un teórico (p. ej. algo sobre "curación de marca"/narrativa corporativa) — si se nombra uno, sourcearlo igual que cualquier claim (`brain/09` A6).
- **Momentos concretos a los que se anclará la reflexión:** la página oficial de Candler (S04) sin mención de la adicción/cocaína/disputa; el contraste "una flor/bebida por una imagen pulida" vs. lo que documenta la historiografía (Pendergrast + la enciclopedia estatal).

## 9. Para llevar — validación

- **Idea aplicable:** cuando una marca o institución cuenta su propio origen, la pregunta útil no es "¿es mentira?" sino "¿qué se quedó fuera, y por qué esa parte en concreto?"
- **¿Se deduce del caso tal como lo contamos?** Sí — es exactamente lo que muestra el contraste S04 vs. S01/S02/S05: la página oficial omite la adicción, la cocaína y la disputa de derechos, no por accidente sino porque esas partes no sirven a la historia que la marca quiere contar.
- **¿Requiere torcer algún hecho?** No — el patrón (venta caótica de derechos, adicción, muerte en la pobreza) está en la literatura secundaria; lo único pendiente es subir de tier los detalles finos (C1, C2, C4-C6, C9), no la forma del takeaway.

## 10. Necesidades de archivo / derechos

| Visual necesario | Posible fuente | Estado de derechos (dominio público / CC / licencia / cita) |
|--------------------|-------------------|-------------------------------------------------------------|
| Retrato de John Pemberton | Wikimedia Commons / New Georgia Encyclopedia | dominio público (pre-1929) — confirmar en Stage 7 |
| Retrato de Asa Candler | Wikimedia Commons | dominio público (pre-1929) — confirmar en Stage 7 |
| Anuncios de época de "French Wine Coca" / primer Coca-Cola | Library of Congress — Chronicling America, Wikimedia Commons | dominio público (prensa pre-1929) |
| Documento de incorporación de The Coca-Cola Company (marzo 1888) | Archivos de Georgia / registros corporativos históricos | dominio público probable — localizar el documento real en Stage 7 |
| Escenas de farmacia / mezcla del jarabe (no hay imagen en movimiento de la época) | recreación / ilustración propia (`brain/15`) | IA — rotulada, sin rostro real fotorrealista |
| Página web actual de la empresa (S04, citada como objeto) | captura propia de la página pública | uso editorial/comentario — nunca branding decorativo, se cita como lo que dice, con atribución en pantalla |

## Gate Stage 2

- [x] Toda afirmación de carga con ≥1 Tier A/B — aprobado tal cual por Usuario 001; C1, C2, C4, C5, C6, C9 quedan anotados en la sección 6 como Tier C a subir de nivel antes o durante Stage 4 (guion), no como bloqueo de Stage 2
- [x] Puntos disputados identificados (sección 5)
- [x] Sin dependencia de Tier C/D para sostener nada — aprobado tal cual (S04/D es deliberadamente el objeto de contraste, no un respaldo; ver nota arriba sobre C1/C2/C4/C5/C6/C9)
- [x] (Ensayo) — no aplica, es Documental
- [x] Revisado por Usuario 001 — fecha 2026-09-12 (`02-research.html` → `02-research.txt`, aprobado = sí)

**Fold aplicado:** decisiones exportadas desde `02-research.html` (`_exports/stage02.json`) — revisor Usuario 001, aprobado sin ediciones línea a línea. Gate firmado; avanza a Stage 3.
